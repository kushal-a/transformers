...
class AttentionLayer(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        if attn_mask is not None:
            ...
            additive_mask = torch.log(attn_mask)
            dot_product += additive_mask.unsqueeze(0)
        
        # apply softmax, dropout, and use value
        y = self.dropout(F.softmax(dot_product, dim=-1)/torch.sqrt(self.embed_dim))@value
        return y  
...
class MultiHeadAttentionLayer(AttentionLayer):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        query = self.query_proj(query).reshape(N, S, H, D//H).transpose(1,2)
        key = self.key_proj(key).reshape(N, T, H, D//H).transpose(1,2)
        value = self.value_proj(value).reshape(N, T, H, D//H).transpose(1,2)
        ...
        if attn_mask is not None:
            ...
            # Think about what inputs make softmax 0.
            additive_mask = torch.log(attn_mask)
            dot_product += additive_mask.unsqueeze(0).unsqueeze(1)
...
class TransformerDecoder(nn.Module):
    ...
    def get_causal_mask(self, _len):
        ...
        mask = torch.tril(torch.ones(_len,_len)).to(self.device)+1e-5
        return mask
